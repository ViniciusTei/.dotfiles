...
- Read AGENTS.md before reviewing to ground your feedback in the project's actual rules.
...
- Do not rationalize away AGENTS.md violations. If the rule says "ALWAYS", "NEVER", "MUST", or "MUST NOT", flag any deviation regardless of context. There are no exceptions based on perceived intent or severity.
...
- `master`: if the repo uses master, make the same as the `main`. Reveiw all commits since the last merge from `master`
- `HEAD~3`: review the last 6 commits
